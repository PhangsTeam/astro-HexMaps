...
def _build_hfs_mask(mask, line_name, hfs_data, this_data):
    ...
    restfreqs = [f * u.Unit(str(u)) for f, u in
                 zip(hfs_data["hfs_ref_freq"][idx_cols], hfs_data["unit"][idx_cols])]
    hfs_freqs = [f * u.Unit(str(u)) for f, u in
...
def run_products(source, fname, meta, cubes, input_mask, hfs_data):
    ...
    shuff_axis         = [meta.get("NAXIS_shuff",       200),
                          meta.get("CDELT_SHUFF",       4000.0)]
    ...
    if use_mask:
        ...
        del this_data[mask_tag]   # remove it from the table; will be re-added as SPEC_MASK
        ...
        for n_mask_i in range(1, n_mask + 1):
            ...
            this_data["SPEC_MASK_" + line_i] = Column(
                mask_i, unit=u.dimensionless_unscaled,
        ...
        if ref_line_method == "ref+HI":
            if "hi" in line_names:
                ...
                LOG.warning(f"HI not found in PyStructure; "
                          "ignoring ref+HI option.")
    ...
    # HFS mask extension
    lines_hfs = (list(set(hfs_data["hfs_name"]))
                 if (use_hfs_lines and hfs_data is not None) else [])
    if use_hfs_lines and hfs_data is not None:
        for jj in range(n_lines):
            if line_names[jj] in lines_hfs:
                ...
                mask_hfs = _build_hfs_mask(mask.value, line_names[jj], hfs_data, this_data)
                if mask_hfs is not None:
                    this_data[f"SPEC_MASK_{line_names[jj].upper()}"] = Column(
                        mask_hfs, unit=u.dimensionless_unscaled,
    ...
    this_data["SPEC_VAXIS"]      = Column(
        np.array([_vaxis]     * n_pts_total), unit=u.km / u.s,
        description="Velocity axis (km/s)")
    this_data["SPEC_VAXISSHUFF"] = Column(
        np.array([new_vaxis]  * n_pts_total), unit=u.km / u.s,
    ...
    for jj in range(n_lines):
        ...
        this_vaxis  = (this_v0 + (np.arange(n_chan_l) - (this_crpix - 1)) * this_deltav
                       ).to(u.km / u.s)
        ...
        if use_hfs_lines and line_name in lines_hfs:
            ...
            active_mask = this_data[hfs_tag] * u.Unit(1) if hfs_tag in this_data.keys() else mask
        else:
            active_mask = mask
        ...
        if band_ext_val in ("", "nan"):
            ...
            this_data["EMOM0_" + line_name.upper()] = Column(mom_maps["mom0_err"], description=f"Uncertainty: {line_desc} mom0")
            this_data["TPEAK_" + line_name.upper()] = Column(mom_maps["tpeak"],    description=f"{line_desc} peak brightness temperature")
            this_data["RMS_"   + line_name.upper()] = Column(mom_maps["rms"],      description=f"{line_desc} rms noise")
            this_data["MOM1_"  + line_name.upper()] = Column(mom_maps["mom1"],     description=f"{line_desc} intensity-weighted mean velocity (mom1)")
            this_data["EMOM1_" + line_name.upper()] = Column(mom_maps["mom1_err"], description=f"Uncertainty: {line_desc} mom1")
            this_data["MOM2_"  + line_name.upper()] = Column(mom_maps["mom2"],     description=f"{line_desc} velocity dispersion (mom2; {mom_calc[2]})")
            this_data["EMOM2_" + line_name.upper()] = Column(mom_maps["mom2_err"], description=f"Uncertainty: {line_desc} mom2")
            this_data["EW_"    + line_name.upper()] = Column(mom_maps["ew"],       description=f"{line_desc} equivalent width")
            this_data["EEW_"   + line_name.upper()] = Column(mom_maps["ew_err"],   description=f"Uncertainty: {line_desc} EW")
            LOG.info(f"Moments computed for {line_name}.")
        else:
            LOG.info(f"Pre-computed 2D map found for {line_name}; skipping moment computation.")
