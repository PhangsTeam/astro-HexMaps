"""
handlerSources.py — SourceHandler: manages the source list and geometry.
...
_LOG_PREFIX = "[pyStructure] [Sources]"
...
class SourceHandler:
    ...
    source_table : pd.DataFrame
        ...
        defined in handlerKeys.TARGET_COLUMNS.
    sources : list of str
    ...
    def _validate(self):
        ...
        if missing:
            raise ValueError(
                f"{_LOG_PREFIX} [ERROR]  The following sources are not in "
    ...
    def get_source_params(self, source: str) -> dict:
        ...
        if source not in self._index:
            raise KeyError(
                f"{_LOG_PREFIX} [ERROR]  Source '{source}' not found in geometry table."
            )
