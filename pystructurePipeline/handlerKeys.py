"""
handlerKeys.py — KeyHandler: reads and validates all PyStructure key files.
...
class KeyHandler:
    ...
    def _validate_key_dir(self):
        ...
        if not self.key_dir.is_dir():
            raise FileNotFoundError(
                f"{_LOG_PREFIX} [ERROR]  Key directory not found: {self.key_dir}"
            )
    ...
    def _load_master_key(self):
        ...
        if not master_path.exists():
            raise FileNotFoundError(
                f"{_LOG_PREFIX} [ERROR]  master_key.txt not found in {self.key_dir}"
            )
    ...
    def _load_config_key(self):
        ...
        if not config_path.exists():
            raise FileNotFoundError(
                f"{_LOG_PREFIX} [ERROR]  config_key not found: {config_path}"
            )
    ...
    def _load_target_definitions(self):
        ...
        if not geom_path.exists():
            raise FileNotFoundError(
                f"{_LOG_PREFIX} [ERROR]  target_definitions not found: {geom_path}"
            )
    ...
    def _load_data_key(self):
        ...
        **Pass 2** — a simple line-by-line parser reads the tabular sections.
        ...
        later in pipelineRegrid.py are valid regardless of the current working
        directory.
        ...
        if not imaging_path.exists():
            raise FileNotFoundError(
                f"{_LOG_PREFIX} [ERROR]  data_key not found: {imaging_path}"
            )
        ...
        # This is essential so that pipelineRegrid can construct valid file
    ...
    def validate(self) -> bool:
        ...
        for issue in issues:
            print(f"{_LOG_PREFIX} [WARNING]  {issue}")
